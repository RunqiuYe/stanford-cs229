...
\begin{solution}
  \begin{enumerate}[(a)]
    ...
    \begin{enumerate}[i.]
      ...
      \item Decreasing the learning rate over time will make the alogrithm converge for dataset $B$, since in this way the change of $\theta$ converge to $0$.
    ...
    \item Support vector machines, which uses hinge loss, are not vulnerable to datasets like $B$. Recall hinge loss $\ell(\hat{y}) = \max(0, 1-y \cdot \hat{y})$. In this case, $\theta$ is normalized. For linearly separable datasets, SVM will minimize the hinge loss to $0$ and the algorithm will stop.
  \end{enumerate}
\end{solution}
